...
status_t DEMO_CreateDbiIface(void)
{
    ...
    status = DBI_LCDIC_DMA_CreateHandle(&s_dbiIface, &dbiConfig, &s_dbiLcdicDmaPrvData);
    if (kStatus_Success != status)
    {
...
status_t DEMO_CreateDC(void)
{
    ...
    /* DC DBI handle. */
    s_dcDbiHandle.dbiIface = &s_dbiIface;
    ...
    status = ST7796S_Init(&lcdHandle, &st7796sConfig, &s_dbiIface);
    if (kStatus_Success != status)
    {
...
status_t DEMO_CreateDbiIface(void)
{
    ...
    /* Create DBI interface. */
    status = DBI_SPI_DMA_CreateHandle(&s_dbiIface,
            &dbiConfig,
...
status_t DEMO_CreateDC(void)
{
    ...
    /* DC DBI config. */
    status = ILI9341_InitDBI(&lcdHandle, NULL, &s_dbiIface);
    if (kStatus_Success != status)
    {
        PRINTF("Panel init failed\r\n");
        for (;;)
